...
class OrderProvider extends ChangeNotifier {
    ...
    for (final doc in snapshot.docs) {
      ...
      if (context.mounted) {
        tableModel = context
            .read<TableProvider>()
            .allTables
            .firstWhere((x) => x.id == order.tableId) as TableModel;
      }
      order.table = tableModel;
      if (context.mounted) {
        order.orderProducts = await context
            .read<ProductProvider>()
            .fetchProductsByOrderId(order.id);
      }
      orders.add(order);
    }
    return orders;
  ...
  Future<OrderModel?> fetchOrderByTableId(
    ...
    if (context.mounted) {
      tableModel = context
          .read<TableProvider>()
          .allTables
          .firstWhere((x) => x.id == latestOrder.tableId) as TableModel;
    }
    latestOrder.table = tableModel;
    if (context.mounted) {
      latestOrder.orderProducts = await context
          .read<ProductProvider>()
          .fetchProductsByOrderId(latestOrder.id);
    }
  ...
  Future<void> saveOrder(OrderModel order) async {
    ...
    try {
      double totalPrice =
          calculateTotalPrice(order); // Toplam tutarı hesaplayın
